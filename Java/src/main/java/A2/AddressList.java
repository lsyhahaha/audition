...
//        System.out.println(System.getProperty("user.dir"));
        ...
        do {
            ...
            choice = sc.nextInt();
            switch (choice) {
                case 1 : Input(arrayList);//添加
                case 2 : delete(arrayList);//删除
                case 3 : alter(arrayList);//修改
                case 4 : find(arrayList);//查询
                case 5 : show(arrayList);//显示
                case 6 : save(arrayList);//保存
            }
        } while (choice != 0);
        save(arrayList);
    ...
    private static void load(ArrayList<Contact> arrayList) throws IOException {
        ...
        BufferedReader br = new BufferedReader(new FileReader("src/main/java/A2/phone.txt"));
        String line;
    ...
    private static void save(ArrayList<Contact> arrayList) throws IOException {
        BufferedWriter bw = new BufferedWriter(new FileWriter(".\\src\\main\\java\\A2\\phone.txt"));
    ...
    private static void delete(ArrayList<Contact> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            if (arrayList.get(i).getNo().equals(no)) {
                flag=false;
                System.out.println("----已删除编号为" + no + "且姓名为"+arrayList.get(i).getName()+"的读者的信息！-----");
